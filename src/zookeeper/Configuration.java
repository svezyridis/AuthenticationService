...
public class Configuration implements ServletContextListener {
	...
	public static List<Map> getSystems() {
...
class FsWatcher implements Watcher {
	...
	private void watchForFsChanges(Watcher watcher) {
		...
		try {
			Systems.clear();
			List<String> fsChildren = zoo.getChildren(dirpath, watcher);
			for (String fs : fsChildren) {
				Map<String,String> system = null;
				Stat stat = zoo.exists(dirpath+"/"+fs, watcher);
				byte[] data=zoo.getData(dirpath+"/"+fs, watcher, stat);
	...
	public void PublishService(ServletContextEvent sce) {
		...
		try {
	       ...
	       try {
			Stat stat = instance.zoo.exists(authpath, false);
			if(stat==null) {
